 ...
 */
void PIDmotorsSetParameters(PID_PARAMETERS localPID_Parameters, int8_t localMotor)
{
	switch (localMotor)
	{
		case MOTOR_A:
 ...
 */
void PIDmotorsSetKParameters(PID_PARAMETERS localPIDParameters, float localKp, float localKi, float localKd)
{
 ...
 */
PID_PARAMETERS PIDmotorsGetParameters(int8_t localMotor)
{
	switch (localMotor)
	{
		case MOTOR_A:
...
void PIDmotorsProcess(PID_PARAMETERS localPIDParameters, int8_t localMotor)
{
	localPIDParameters.e__ = localPIDParameters.e_;
	localPIDParameters.e_ = localPIDParameters.e;
	localPIDParameters.e = PIDmotorsGetError(localMotor);
	...
	if (localPIDParameters.u > MAX_MOTORS_CONTROL_SIGNAL)
	{
		localPIDParameters.u = MAX_MOTORS_CONTROL_SIGNAL;
	}
	else if (localPIDParameters.u < -MAX_MOTORS_CONTROL_SIGNAL)
	{
		localPIDParameters.u = -MAX_MOTORS_CONTROL_SIGNAL;	
	}
}

float PIDmotorsControlSignal(PID_PARAMETERS localPIDParameters, int8_t localMotor)
{
	PIDmotorsProcess(localPIDParameters,localMotor);
	return localPIDParameters.u;
}

void PIDmotorsResetParameters(int8_t localMotor)
{
	switch (localMotor)
	{
		case MOTOR_A:
...
float PIDmotorsGetError(int8_t localMotor)
{
	return 0;
}
